	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		synchronized(session) {
			...
			if (itemsOrdered.size() == 0) {
				...
				for(ItemOrder orderLine : itemsOrdered) {
					// order = (ItemOrder)itemsOrdered.get(i);
					out.println
							("<TR>\n" +
									"  <TD>" + orderLine.getItemID() + "\n" +
									"  <TD>" + orderLine.getShortDescription() + "\n" +
									"  <TD>" +
									formatter.format(orderLine.getUnitCost()) + "\n" +
									"  <TD>" +
									"<FORM>\n" +  // Submit to current URL
									"<INPUT TYPE=\"HIDDEN\" NAME=\"itemID\"\n" +
									"       VALUE=\"" + orderLine.getItemID() + "\">\n" +
									"<INPUT TYPE=\"TEXT\" NAME=\"numItems\"\n" +
									"       SIZE=3 VALUE=\"" +
									orderLine.getNumItems() + "\">\n" +
									"<SMALL>\n" +
									"<INPUT TYPE=\"SUBMIT\"\n "+
									"       VALUE=\"Update Order\">\n" +
									"</SMALL>\n" +
									"</FORM>\n" +
									"  <TD>" +
									formatter.format(orderLine.getTotalCost()));
				}
				String checkoutURL = response.encodeURL("/cores1/store1/checkout.html");
